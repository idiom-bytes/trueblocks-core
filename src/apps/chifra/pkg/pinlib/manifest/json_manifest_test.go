...
func TestReadManifestFailsForWrongVersion(t *testing.T) {
	wrongSource := strings.Replace(manifestSource, `"version": "2"`, `"version": "12345"`, 1)

	m, err := ReadJSONManifest(strings.NewReader(wrongSource))
	if err == nil {
		t.Error("no error", m.Version.String())
	}
